...
{
	// function to print a sequence of 2D points in 2D plane,
	// given the number of elements and the actual sequence
	// stored as an array of 2D points
	// n is the number of points
	// seq is a permutation over the set of indexes
...
float farthest(int n, point2D *P)  // farthest length permutation
{                                  // initially set as the best option
								   //function to calculate the furthest
								   //distance between any two 2D points
...
void print_perm(int n, int *A, int sizeA, point2D *P, int *bestSet, float &bestDist)
{
	...
	if (n == 1)
	{
		//we obtain a permutation so we compare
		// it against the current shortest
		//Hamiltonian cycle
		// You NEED TO COMPLETE THIS PART
		// *bestSet will need to change

		for (i = 0; i < sizeA - 1; i++)
		{
			sum = sqrt(((P[A[i]].x - P[A[i + 1]].x)*(P[A[i]].x - P[A[i + 1]].x)) +
				((P[A[i]].y - P[A[i + 1]].y)*(P[A[i]].y - P[A[i + 1]].y)));
			dist = dist + sum;
		}
		dist = dist + sqrt(((P[A[sizeA - 1]].x - P[A[0]].x)*(P[A[sizeA - 1]].x - P[A[0]].x))
			+ ((P[A[sizeA - 1]].y - P[A[0]].y)*(P[A[sizeA - 1]].y - P[A[0]].y)));
...
int main()
{
	...
	// Start the chronograph to time execution of the algorithm
	...
	cout << "\nThe Hamiltonian cycle of the minimum length: " << endl;
